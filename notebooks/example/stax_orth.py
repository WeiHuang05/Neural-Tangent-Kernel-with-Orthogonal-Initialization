...
@_layer
def Dense_orth(out_dim, W_std=1., b_std=0., W_init=_randn(1.0), b_init=_randn(1.0),
          parameterization='ntk'):
  r"""Layer constructor function for a dense (fully-connected) layer.
  ...
  def De_Dense_or(out_dim, W_init, b_init ):
    ...
    def init_fun(rng, input_shape):
      ...
      #print ('---------------')
      #print (np.dot(W,W.T))
      #print (np.dot(W,W.T).shape) 
      #print (np.dot(W.T,W))
      #print (np.dot(W.T,W).shape)
      #print (input_shape[-1],out_dim)
      #print ('-----------')
      shapesize = np.max([input_shape[-1], out_dim])
      #print (shapesize)
      W = np.sqrt(shapesize)*W
  ...
  ntk_init_fn = De_Dense_or(out_dim, orthogonal(), _randn(1.0))
  ...
  #train_points = 100
  #train_xs  = random.normal(x_key, (train_points,10000))*1.0**0.5
  #_, params = ntk_init_fn(key, input_shape=train_xs.shape)
  #_, params1 = ntk_init_fn(x_key, input_shape=train_xs.shape)

  #print ('------------')
  #fx2_struct = eval_shape(apply_fn, params, train_xs)
  #fx_dummy = np.ones(fx2_struct.shape, fx2_struct.dtype)
 

  #def delta_vjp_jvp(delta):
  #  def delta_vjp(delta):
  #    return vjp(lambda p: apply_fn(p, train_xs), params1)[1](delta)
  #    
  #  return jvp(lambda p: apply_fn(p, train_xs), (params1,), delta_vjp(delta))[1]
  #
  #vjp_temp = vjp(lambda p: apply_fn(p, train_xs), params1)[1](fx_dummy)
  #jvp_temp = jvp(lambda p: apply_fn(p, train_xs), (params,), vjp(lambda p: apply_fn(p, train_xs), params)[1](fx_dummy))[1]

  #print (jvp_temp)
  #ntk = jacobian(delta_vjp_jvp)(fx_dummy)
  #print (1.13103448*np.dot(train_xs,vjp_temp[0][0])+0.1767677*vjp_temp[0][1])
  #print (ntk.reshape(100,100))
